...
interface InstallmentPlan {
  ...
  installment_types: InstallmentLine[];
}

interface EditInstallmentPlanProps {
  selectedPlan: InstallmentPlan;
  setShowEditModal: React.Dispatch<React.SetStateAction<boolean>>;
}

const EditInstallmentPlan = ({ selectedPlan, setShowEditModal }: EditInstallmentPlanProps) => {
  const [name, setName] = useState(selectedPlan?.name || "");
  const [lines, setLines] = useState<InstallmentLine[]>([]);

  useEffect(() => {
    if (selectedPlan && selectedPlan.installment_types) {
      setLines(
        selectedPlan.installment_types.map((line) => ({
          name: line.name,
          amount: line.amount,
  ...
  const editPlan = useCustomUpdate(() => `/cards/installments/${selectedPlan.id}/`, [
    "installments",
  ]);
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        <div className="p-6 border-b border-gray-200 sticky top-0 bg-white z-10">
          <div className="flex items-center justify-between">
            <h2 className="text-xl font-bold text-gray-800">تعديل خطة التقسيط</h2>
            <button
        ...
        <div className="p-6 border-t border-gray-200 sticky bottom-0 bg-white flex gap-3 justify-end z-10">
          ...
          <button
            ...
            disabled={!name.trim() || editPlan.isPending}
            className={`px-6 py-2 rounded-lg transition-all flex items-center gap-2
                ${
                  !name.trim() || editPlan.isPending
                    ? "bg-gray-300 text-gray-500 cursor-not-allowed"
                    : "btn-brand-slide text-white"
                }`}
          >
            <Save size={16} />
            {editPlan.isPending ? "جاري الحفظ..." : "تحديث الخطة"}
          </button>
        </div>
      </div>
    </div>
