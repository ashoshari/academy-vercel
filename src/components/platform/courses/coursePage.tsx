...
const CoursePage: React.FC = () => {
  ...
  const [examResults, setExamResults] = useState<{score: number; passed: boolean; answers: {[key: number]: {selected: number; correct: number}}} | null>(null);
  ...
  const sampleExam: Exam = {
    ...
    questions: [
      ...
      },
      {
        ...
        correctAnswer: 1,
        explanation: "المعادلة y = 2x + 3 هي معادلة خط مستقيم بالصيغة y = mx + b"
      },
      {
        id: 3,
  ...
  useEffect(() => {
            ...
            lessons: [
              {
                ...
                files: [
                  { id: 1, name: "ملخص الدرس.pdf", type: "pdf", size: "2.1 MB", url: "#" },
                  { id: 2, name: "أمثلة إضافية.doc", type: "doc", size: "1.5 MB", url: "#" }
                ]
              },
              {
                id: 2,
                ...
                files: [
                  { id: 3, name: "تمارين الدرس.pdf", type: "pdf", size: "1.8 MB", url: "#" }
                ]
              },
              {
                id: 3,
    ...
    const initialCourseFiles: CourseFile[] = [
      {
        ...
        downloads: 1250,
        description: "دليل شامل يحتوي على جميع المواضيع والقوانين المهمة في الدورة",
        url: "#"
      },
      {
        id: 2,
    ...
    const initialNotes: Note[] = [
      {
        id: 1,
        content: "تذكر: عند حل المعادلات الخطية، يجب دائماً التحقق من الحل بالتعويض",
      ...
      {
        id: 2,
        content: "ملاحظة مهمة: القوانين الأساسية للجبر ستحتاجها في جميع الوحدات القادمة",
    ...
    const initialQuestions: CourseQuestion[] = [
      {
        id: 1,
        title: "كيفية التعامل مع المعادلات المعقدة؟",
        content: "أواجه صعوبة في حل المعادلات التي تحتوي على كسور. هل يمكن توضيح الطريقة الصحيحة؟",
        author: "أحمد محمد",
        ...
        answers: [
          {
            id: 1,
            content: "الطريقة الأفضل هي ضرب جميع الحدود في المضاعف المشترك الأصغر للمقامات لإزالة الكسور أولاً",
            author: "سارة أحمد",
          ...
          {
            id: 2,
            content: "يمكنك أيضاً التعامل مع كل كسر على حدة، لكن الطريقة الأولى أسرع وأقل عرضة للأخطاء",
            author: "محمد علي",
      ...
      },
      {
        ...
        answers: [
          {
            id: 3,
            content: "حسب ما فهمت من الدرس، سيركز على المعادلات الخطية فقط في هذه الوحدة",
            author: "عمر حسن",
  ...
  const handleExamSubmit = () => {
    ...
    const answerDetails: {[key: number]: {selected: number; correct: number}} = {};
  ...
  const handleAddQuestion = () => {
    if (newQuestionTitle.trim() && newQuestionContent.trim()) {
      ...
      setCourseQuestions(prev => [question, ...prev]);
      setNewQuestionTitle('');
  ...
  const handleAddAnswer = (questionId: number) => {
    if (newAnswer.trim()) {
      ...
      setCourseQuestions(prev => prev.map(q => 
        q.id === questionId 
  ...
  const handleApproveAnswer = (questionId: number, answerId: number) => {
    setCourseQuestions(prev => prev.map(q => 
      q.id === questionId 
        ? {
            ...q, 
  ...
  const renderSidebar = () => (
    ...
    } ${sidebarVisible ? 'translate-x-0' : '-translate-x-full'} fixed left-0 top-0 h-full z-40 overflow-y-auto`}>
      {/* Sidebar Header */}
      <div className="p-4 border-b border-gray-200 bg-gradient-to-r from-blue-50 to-purple-50">
        <div className="flex items-center justify-between">
          {!sidebarCollapsed && (
            <div className="flex-1">
              <h3 className="font-bold text-gray-900 text-sm line-clamp-2">{courseData.title}</h3>
              <p className="text-xs text-gray-600 mt-1">{courseData.instructor}</p>
            </div>
          )}
      ...
      <div className="p-2">
        {chapters.map((chapter) => (
          <div key={chapter.id} className="mb-2">
            <button
              onClick={() => !sidebarCollapsed && toggleChapter(chapter.id)}
            ...
            {chapter.isExpanded && !sidebarCollapsed && (
              <div className="ml-4 mt-2 space-y-1">
                {chapter.units.map((unit) => (
                  <div key={unit.id}>
                    ...
                    >
                      <span className="font-medium text-gray-700 text-sm text-right flex-1">
                        {unit.title}
                      </span>
                      {unit.isExpanded ? (
                        ...
                        {unit.lessons.map((lesson) => {
                          ...
                          return (
                            <button
                              ...
                              className={`w-full flex items-center space-x-3 p-2 rounded-lg transition-all duration-200 text-right ${
                                isCurrentLesson
    ...
    <div className="bg-white rounded-2xl shadow-lg overflow-hidden">
      <div className="aspect-video bg-black rounded-t-2xl overflow-hidden">
        <iframe
          width="100%"
          height="100%"
          src={`https://www.youtube.com/embed/${currentLesson.videoUrl}?autoplay=1&rel=0`}
      ...
      <div className="p-6">
        ...
        {currentLesson.files && currentLesson.files.length > 0 && (
          <div className="mb-6">
            <h3 className="text-lg font-semibold text-gray-900 mb-3">ملفات الدرس</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              {currentLesson.files.map((file) => (
                <div key={file.id} className="flex items-center space-x-3 p-3 bg-gray-50 rounded-xl hover:bg-gray-100 transition-colors duration-200">
        ...
        <div className="flex items-center justify-between">
          ...
          >
            ...
            <span>{currentLesson.isCompleted ? 'مكتمل' : 'وضع علامة مكتمل'}</span>
          </button>
        </div>
      </div>
    </div>
  ...
  const renderFileContent = () => (
    <div className="bg-white rounded-2xl shadow-lg overflow-hidden">
      <div className="p-6">
        ...
        {currentLesson.files && currentLesson.files.length > 0 && (
          <div className="mb-6">
            <h3 className="text-lg font-semibold text-gray-900 mb-4">ملفات الدرس</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {currentLesson.files.map((file) => (
                <div key={file.id} className="flex items-center space-x-3 p-4 bg-gray-50 rounded-xl hover:bg-gray-100 transition-colors duration-200 border border-gray-200">
                  <div className="w-10 h-10 bg-blue-100 rounded-lg flex items-center justify-center">
                    <FileText className="w-5 h-5 text-blue-500" />
                  </div>
                  <div className="flex-1">
                    <div className="font-medium text-gray-900">{file.name}</div>
                    <div className="text-sm text-gray-500">{file.type.toUpperCase()} • {file.size}</div>
                  </div>
                  <button className="p-2 text-blue-600 hover:bg-blue-50 rounded-lg transition-colors duration-200">
                    <Download className="w-4 h-4" />
                  </button>
        ...
        <div className="mb-8 p-6 bg-gray-50 rounded-xl">
          <div className="text-center py-12">
            ...
            <h3 className="text-lg font-semibold text-gray-700 mb-2">محتوى الدرس</h3>
            <p className="text-gray-500 mb-4">قم بتحميل الملفات المرفقة لمراجعة محتوى الدرس</p>
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-lg p-8">
          <div className="text-center mb-8">
            ...
            <p className="text-xl text-gray-600 mb-4">
              درجتك: <span className={`font-bold ${examResults.passed ? 'text-green-600' : 'text-red-600'}`}>
                {examResults.score}%
              </span>
            </p>
    ...
    return (
      <div className="bg-white rounded-2xl shadow-lg p-8">
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            <h2 className="text-2xl font-bold text-gray-900">{currentExam.title}</h2>
            <p className="text-gray-600">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="text-center">
            <div className="text-sm text-gray-600 mb-2">
              تم الإجابة على {Object.keys(selectedAnswers).length} من {currentExam.questions.length} أسئلة
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        ...
        <div className="bg-gradient-to-r from-orange-500 to-red-500 rounded-2xl p-6 text-white">
          <div className="flex items-center space-x-3 mb-4">
            ...
            <div>
              <div className="text-3xl font-bold">
                {allLessons.length - allLessons.filter(l => l.isCompleted).length}
              </div>
      ...
      <div className="space-y-4">
        ...
        {chapters.map((chapter) => (
          <div key={chapter.id} className="border border-gray-200 rounded-xl p-4">
            <h4 className="font-semibold text-gray-900 mb-3">{chapter.title}</h4>
            {chapter.units.map((unit) => {
    ...
    <div className="bg-white rounded-2xl shadow-lg p-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {courseFiles.map((file) => (
          <div key={file.id} className="border border-gray-200 rounded-xl p-6 hover:shadow-lg transition-all duration-300 group">
    ...
    <div className="bg-white rounded-2xl shadow-lg p-8">
      ...
      {showAddNote && (
        <div className="bg-gray-50 rounded-xl p-6 mb-6">
          <h3 className="font-semibold text-gray-900 mb-4">إضافة ملاحظة جديدة</h3>
          <textarea
            value={newNote}
      ...
      <div className="space-y-4">
        {notes.length === 0 ? (
          ...
          notes.map((note) => (
            <div key={note.id} className="border border-gray-200 rounded-xl p-6 hover:shadow-lg transition-all duration-300">
              <div className="flex items-start justify-between mb-4">
                <div className="flex-1">
              ...
              <div className="space-y-4 mb-4">
                {question.answers.map((answer) => (
                  <div key={answer.id} className={`p-4 rounded-xl border ${
                    answer.isTeacherApproved ? 'border-green-200 bg-green-50' : 'border-gray-200 bg-gray-50'
                  }`}>
                    ...
                    <div className="flex items-center space-x-4">
                      ...
                      >
                        ...
                        <span>{answer.isTeacherApproved ? 'معتمد' : 'اعتماد'}</span>
                      </button>
                    </div>
                  </div>
                ))}
            ...
            <div className="space-y-8">
              ...
              <div className="flex items-center justify-between bg-white rounded-2xl shadow-lg p-6">
                ...
                <button
                  ...
                  disabled={currentLessonIndex === allLessons.length - 1 || allLessons[currentLessonIndex + 1]?.isLocked}
